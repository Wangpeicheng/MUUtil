...
public class MiracleEventCenter
{
    public delegate void EventHandler<T>(object sender, T evt);

    public static MiracleEventCenter Instance
    {
        get
        {
            if (null == instance)
            {
                instance = new MiracleEventCenter();
            }
            return instance;
        }
    }

    private static MiracleEventCenter instance;
    ...
    public bool ContainsHandlers(Type eventType)
    {
