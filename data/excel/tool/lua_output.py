...
def read_xls_data(fen,err):
    ...
    try:
        ...
        for col in range(0, table.ncols):
            ...
            # 检查key是否在fen配置中配置
            keyConfList.append(check_in_file_column_list(key, fen))
...
def read_all_xls_data(pconf):
    ...
    for fen in pconf.file_list:
        ...
        tempPath = os.path.join(pconf.xls_path,fen.xls_file)
        if tempPath not in global_change_files:
            continue

        ret, oerr = read_xls_data(fen, err)
        if ret != True:
            return False, oerr
...
def output(pconf):
    ...
    for f in pconf.file_list:
        #没有改变的表跳过
        tempPath = os.path.join(pconf.xls_path,f.xls_file)
        if tempPath not in global_change_files:
           continue;
        ...
        #print("hello")
        if bin_ret != True:
            print("%s,%s output_lua failed, err:\r\n" % (f.xls_file, f.sheet_name))
            print(oerr.get_err_msg())
            return False, oerr
        else:
            print("%s,%s output_lua ok." % (f.xls_file, f.sheet_name))
...
def output_bin(fconf, data_path, xls_path):
    ...
    #print(data_path)
    data_path = "./lua_out"
    fout_path = os.path.join(data_path, fconf.target_name + ".lua")
    try:
        ...
        for row in table:
            ...
            for col in col_list:
                ...
                if not row.has_key(col[0]):
                    ...
                    return False, err
                value = row[col[0]]
                #print(value + "--row=" + row[1] + "--col" + col[1])
                ...
                if col[2] != "":
                    ...
                    if replace_ret != True:
                        err.output_err_msg = "line(%s) replace refer_enum(%s) value(%s) failed." % (row_line+2, col[2], value)
                        return False, err
                ...
                if col[1] == "Number" or col[1] == "Number64":
                    if value.strip() == "":
                        value = "0.0"
                ...
                if col_line == 1:
                    wirteData = wirteData + '\t' + '[' + data + ']' + ' = ' + '{ '
                else:
                    wirteData = wirteData + col[len(col) - 1] + ' = ' + data + ','
            else:
                wirteData = wirteData + '},\n'
        else:
            wirteData = wirteData + '}\n\n'
...
def check_cell(value, check_list, pconf):
    for check in check_list:
        if check.type == "num_limit":
            ...
            if table_dic == None: #试着重新加载一次
                err = OutputError()
                read_xls_data(fconf,err)
                table_dic = get_xls_dic(xls_file, sheet_name)
...
if __name__ == "__main__":
    ...
    compare_file.log_compare(pconf.xls_path,pconf.file_list,global_change_files,outtype,conf_file)
    ...
    compare_file.add_log(pconf.xls_path,pconf.file_list,outtype,conf_file)
